...
public class AiChatService {
    ...
    public void streamChat(ChatRequest request, SseEmitter emitter) {
        try {
            ...
            try {
                emitter.send(SseEmitter.event().data(Map.of("type", "error",
                        "content", "AI 服务暂时不可用: " + e.getMessage())));
    ...
    private String streamFinalAnswer(WebClient wc, List<Map<String, Object>> msgs,
        ...
        wc.post().uri("/v1/chat/completions")
    ...
    private List<Map<String, Object>> buildMessages(ChatRequest req, boolean includeSchema) {
        ...
        // 当前用户消息：如果有图片，用多模态格式；否则纯文本
        msgs.add(buildUserMessage(req));
        return msgs;
    }

    /**
     * 构建用户消息：支持纯文本 / 文本+图片（多模态）
     */
    private Map<String, Object> buildUserMessage(ChatRequest req) {
        String text = req.getMessage() == null ? "" : req.getMessage();
        ...
        // 有图片 → 多模态消息（OpenAI 兼容格式）
        List<Map<String, Object>> content = new ArrayList<>();
        if (isNotBlank(text)) {
            content.add(Map.of("type", "text", "text", text));
        } else {
            // 只有图片没有文字时，加个默认提示让模型描述图片
            content.add(Map.of("type", "text", "text", "请描述这张图片，并结合业务场景回答用户可能的问题。"));
        }
